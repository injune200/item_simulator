...
router.get("/character/:character_id", async(req, res, next) => { //캐릭터 id 단일 조회
    const {character_id} = Number(req.params);

    const character = await Character_infor.findOne(character_id).exec();
...
router.post("/character", async(req, res, next) =>{ //캐릭터 생성
    ...
    return res.status(201).json({character_infor : character});
})
...
router.delete("/character/:character_id",async(req,res,next) => { //캐릭터 삭제
    ...
    await Character_infor.deleteOne({_id : character._id});
